...
if __name__ == "__main__":
    ...
    # Werte CLI-Parameter aus (Serververbindung noetig)
        ...
        m.workflows.templateNordsee(args.tnds)
    elif args.taak != None:
        # Template Antarktisdaten
        m.workflows.templateAntarktis(args.taak)
    elif args.tark != None:
        ...
        # Workflow Nordseedaten
        if not os.path.isfile(args.nds[0]):
            print("Eingabedatei '"+args.nds[0]+"' existiert nicht... Abbruch")
            quit()
        m.workflows.ingestNordsee(args.nds[0])
    elif args.aak != None:
        # Workflow Antarktisdaten
        if not os.path.isfile(args.aak[0]):
            print("Eingabedatei '" + args.aak[0] + "' existiert nicht... Abbruch")
            quit()
        m.workflows.ingestAntarktis(args.aak[0])
    elif args.ark != None:
        # Workflow Arktis
