...
class View(QMainWindow, uic.loadUiType(fileHauptfenster)[0]):
    ...
    def __init__(self, model):
        ...
        self.setupUi(self)
            # Bilder
        self.sPathPicCrabby = os.path.dirname(os.path.realpath(__file__)) + "ressources/crabby.png"
            # Window Icon
        self.setWindowIcon(QIcon(self.sPathPicCrabby))
        self.guiRefeshWindowTitelDryRun()
            # Actions
        self.actionNordsee.triggered.connect(self.actionNordseeClicked)
        self.actionArktis.triggered.connect(self.actionArktisClicked)
        self.actionAntarktis.triggered.connect(self.actionAntarktisClicked)
        self.actionTemplateNordsee.triggered.connect(self.actionTemplateNordseeClicked)
        self.actionTemplateArktis.triggered.connect(self.actionTemplateArktisClicked)
        self.actionTemplateAntarktis.triggered.connect(self.actionTemplateAntarktisClicked)
    ...
    # Ingest Nordsee
    def actionNordseeClicked(self):
        if not self.__model.workflows.isReady():
            return
        csvFile = QFileDialog.getOpenFileName(self, "CSV-Datei", filter="*.csv;; *.*")[0]  # Hole CSV-Datei
        if csvFile != "":
            bOk = self.__model.workflows.ingestNordsee(csvFile) # Starte Workflow
            if bOk == False:
                self.popupFehler("Fehler","Es gab Probleme bei der Ingestion.\nNäheres siehe Konsolenausgabe ...")

    # Template Nordsee
    def actionTemplateNordseeClicked(self):
        if not self.__model.workflows.isReady():
            return
        excelFile = QFileDialog.getSaveFileName(self, "Speichere Excel-Datei", "template_nordsee.xlsx")[0]
        if excelFile != "":
            bOk = self.__model.workflows.templateNordsee(excelFile)
            if bOk == False:
                self.popupFehler("Fehler", "Es gab Probleme beim Erzeugen des Templates.\nNäheres siehe Konsolenausgabe ...")

    # Ingest Antarktis
    def actionAntarktisClicked(self):
        if not self.__model.workflows.isReady():
            return
        csvFile = QFileDialog.getOpenFileName(self, "CSV-Datei", filter="*.csv;; *.*")[0]  # Hole CSV-Datei
        if csvFile != "":
            bOk = self.__model.workflows.ingestAntarktis(csvFile)  # Starte Workflow
            if bOk == False:
                self.popupFehler("Fehler", "Es gab Probleme bei der Ingestion.\nNäheres siehe Konsolenausgabe ...")

    # Template Antarktis
    def actionTemplateAntarktisClicked(self):
        if not self.__model.workflows.isReady():
            return
        excelFile = QFileDialog.getSaveFileName(self, "Speichere Excel-Datei", "template_antarktis.xlsx")[0]
        if excelFile != "":
            bOk = self.__model.workflows.templateAntarktis(excelFile)
            if bOk == False:
                self.popupFehler("Fehler","Es gab Probleme beim Erzeugen des Templates.\nNäheres siehe Konsolenausgabe ...")
