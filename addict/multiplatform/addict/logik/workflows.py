...
try:
    import os, csv, psycopg2, xlsxwriter, getpass, platform, datetime, time
...
- Die CSV wird von links nach rechts durchlaufgen (wichtig bei Auswertung der Trigger-Rückgabewerte!)
- Zwischen zwei Triggern befinden sich die Daten für einen Datensatz (Eintrag in eine Tabelle)
- Ist Trigger gedrückt wird ein neuer Eintrag erzeugt (Trigger-Return: Id)
- Ist er nicht aktiviert wird der alte Eintrag anhand der Id nochmal benutzt (ReUse)
- Pro Trigger wird immer ein gesamter Datensatz erzeugt (Datensätze die aus zwei gedrückten Triggern bestehen werden nicht unterstützt)
- Ein Trigger gilt als gedrückt wenn ...
  - Triggerfeld leer
  - Triggerfeld gleich dem alten (Grund: Zeilen dürfen doppelt in der CSV stehen, es wird aber kein neuer Datensatzerzeugt sondern ReUse)
  - (Strictmode: alle Felder für einen Datensatz sind gleich den vorherigen)
  - Evtl. Problem: Dinge nach gleichem Trigger werden nicht beachtet (ist aber auch nicht sehr sinnvoll)
- Ist ein Trigger gedrückt ist ein ReUse danach für alle Datensätze daktiviert (alles muss neu eingelesen werden)
...
class Workflows:
    ...
    def __init__(self, model):
        ...
        # Vom Workflow unabhängige Spezialanpassungen
        # --------------------------------------------------------------------
        self.__dicSpezialAid2Name = None # Taxon: Aid -> Taxon
        # --------------------------------------------------------------------
    ...
    def initialisierung(self):
        ...
        pnProgress(9, iMax, iMax, guiText=sInfo + "erzeuge Wörterbuch: Taxon-Aid/Name", abs=False, cli=False)
        bOkSpecialTaxonAid2Name = self.__buildSpecialTaxonAid2Name()

        # Gab es Fehler?
        self.__bReady = self.bOkWorkflowModell and bOkConnection and bOkPopup and bOkObligate and bOkCsv and bOkTable and bOkTrigger and bOkTrigDat and bOkLookup and bOkSpecialTaxonAid2Name
    ...
    def info(self):
        ...
        pn("TaxAidName: " + str(len(self.__dicSpezialAid2Name)))
        for k, v in self.__dicSpezialAid2Name.items():
            pd("\t\t" + str(k) + ":" + str(v))
    ...
    def __buildSpecialTaxonAid2Name(self):
        ...
        try:
            ...
            self.__dicSpezialAid2Name = dict((str(name).lower(), id) for name, id in lstAntwort)
                        ...
                        for sTrigName, bTrig in odicTrig.items(): # Alle Trigger durchlaufen
                            if bTrig: # Ist Trigger aktiviert?
                                ...
                                # ---
                                    ...
                                    try:
                                        # Viele Zugriffe auf Dictionaries, die nicht ins Leere führen dürften ...
                                        if sTrigName == "taxon_aid":
                                            iTestTaxonId = None
                                            sTestTaxonAid = None
                                            sTestTaxonNameEingabe = None
                                            sTestTaxonNameAusTaxonliste = None
                                            # Hole Aid und Name aus aus Eingabe
                                            iTestTaxonId = dicAktZeile["taxon_aid"] # Id (Aid ist schon umgerechnet)
                                            sTestTaxonAid = self.__dicCsv2LookupReverse["taxon_aid"][iTestTaxonId] # Id -zurück zu-> Aid
                                            sTestTaxonNameEingabe = str(dicAktZeile["taxon_name"]).lower()
                                            sTestTaxonNameAusTaxonliste = str(self.__dicSpezialAid2Name[sTestTaxonAid]).lower()
                                        ...
                                        pn("Glückwunsch! "+
                                           "Es wurde beim Taxon-Aid/Namen-Test ein äußerst seltenes technisches Problemchen endeckt: "+
                                           "Zeile: "+str(iZeile)+
